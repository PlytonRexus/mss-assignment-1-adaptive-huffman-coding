...
from .utils import (bin_str2bool_list,
...
class AdaptiveHuffman:
    def __init__(self, byte_seq, alphabet_range=(0, 255)):
        ...
        alphabet_size = abs(alphabet_range[0] - alphabet_range[1]) + 1      # m
        # Select an `exp` and `rem` which meet `alphabet_size = 2**exp + rem`.
        # Get the largest `exp` smaller than `alphabet_size`.
        self.exp = alphabet_size.bit_length() - 1                           # e
        self.rem = alphabet_size - 2**self.exp                              # r
    ...
    def encode(self):
        ...
        for symbol in self.byte_seq:
            ...
            # send code which is path from root to the node of symbol or the NYT code
    ...
    def decode(self):
        ...
        # This extra data is the padding added by the tofile() function to make
        # the data bits a multiple of 8.
